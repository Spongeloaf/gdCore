...
public static class TrajectoryPlotter
{
    ...
    public static int SolveQuartic(double c0, double c1, double c2, double c3, double c4, out double s0, out double s1, out double s2, out double s3)
    {
        ...
        if (IsZero(r))
        {
            ...
            num = TrajectoryPlotter.SolveCubic(coeffs[0], coeffs[1], coeffs[2], coeffs[3], out s0, out s1, out s2);
        }
        else
        {
            ...
            num = TrajectoryPlotter.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);

            coeffs[2] = z + u;
            coeffs[1] = q < 0 ? v : -v;
            coeffs[0] = 1;

            if (num == 0) num += TrajectoryPlotter.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s0, out s1);
            else if (num == 1) num += TrajectoryPlotter.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s1, out s2);
            else if (num == 2) num += TrajectoryPlotter.SolveQuadric(coeffs[0], coeffs[1], coeffs[2], out s2, out s3);
        }
    ...
    public static float ballistic_range(float speed, float gravity, float initial_height)
    {
        ...
        float angle = Mathf.DegToRad(45); // no air resistence, so 45 degrees provides maximum range
    ...
    {
        if (proj_pos == target_pos || lateral_speed == 0 || max_height <= proj_pos.Y)
        {
            Log.GameError("solve_ballistic_arc called with invalid data");
    ...
    public static bool solve_ballistic_arc_lateral(Vector3 proj_pos, float lateral_speed, Vector3 target, Vector3 target_velocity, float max_height_offset, out Vector3 fire_velocity, out float gravity, out Vector3 impact_point)
    {
        ...
        float c0 = targetVelXZ.Dot(targetVelXZ) - lateral_speed * lateral_speed;
        float c1 = 2f * diffXZ.Dot(targetVelXZ);
        float c2 = diffXZ.Dot(diffXZ);
        double t0, t1;
        int n = TrajectoryPlotter.SolveQuadric(c0, c1, c2, out t0, out t1);
